...
class HelpTheme(tk.Toplevel):
    ...
    def __init__(self, widget, static = False, *args, **kw):
        self._configs = {
            'text'       : 'Th== text will be showed when required',
            'color'      : 'black',
            'background' : '#ffffcc',
            'font'       : ('arial', 9, 'italic'),
            'padding'    : 5,
...
class EntryTheme(ttk.Entry):
    ...
    def _configs_update(self, kw):
        ...
        for key in list(kw.keys()):
            if key in self._configs.keys():
                if key == 'type' and kw[key] not in ['text', 'password', 
        ...
        if self._configs['maxlength'] != None and \
    ...
    def insert(self, index, text, validate=True):
        ...
        if text != None: super().insert(index, text)
...
class RadiobuttonTheme(ttk.LabelFrame):
    ...
    def set_value(self, value): self.value.set('' if  value == None else value)
...
class OptionMenuTheme(ttk.OptionMenu):
    ...
    def how_it_works():
        def fun():
            print("OP1 - When 'items' == a Dict get_value return: ", widget.get_value(),
                  "\nOP2 - When 'items' == a List get_value return: ", widget2.get_value(), "\n")
...
class CheckButtonTheme(ttk.Checkbutton):
    def __init__(self, master, text, callback=None, *args, **kw):
        if 'textvariable' in kw:
            raise(AttributeError('option -textvariable != valid for th== widget!\
    ...
    def disable(self): self.configure(state= tk.DISABLED, cursor= 'arrow')
    def get_value(self): return self.value.get()
    def set_value(self, value): self.value.set(False if value == None else value)
...
class PickerTheme(ttk.Frame):
    ...
    def how_it_works():
        ...
        picker = PickerTheme(root, 'file', title = 'Title == Choose a file', 
                initialdir = '/')
        picker.pack(side='top')
...
class SimpleCalendar(ttk.Entry):
    ...
    def set_value(self, value = None, format_show = None):
        ...
        if isinstance(value, date):
            ...
            if re.match(r'^([0-9]{2}|[0-9]{1})\/([0-9]{2}|[0-9]{1})\/[0-9]{4}$', value) != None:
                # Exception not documented date input like "yyyy/mm/dd"
                value = value.split('/')
                self._date = date(int(value[2]), int(value[1]), int(value[0]))
                
            elif re.match(r'^[0-9]{4}\-([0-9]{2}|[0-9]{1})\-([0-9]{2}|[0-9]{1})$', value) != None:
                # ISO Date
        ...
        if format_show != None: self._optns['format'] = format_show
...
class FormTheme(object): 
    ...
    def add_entry(self, name, label=None, *args, **kw):
        ...
        if options['value'] != None: widget.set_value(options['value'])
    ...
    def add_link(self, text, command = None,  *args, **kw):
        ...
        if command != None: widget.bind("<Button-1>", lambda e: command())
        ...
        for item in state_map.keys():
            ...
            for subitem in state_map[item].keys():
                assert subitem in self._elements.keys() \
                    or subitem in self._buttons.keys(), f"item -{subitem} != defined!"
    ...
    def set_state(self, state_name):
        ...
        if state_name not in ['all_active', 'all_disabled', \
            ...
            for name, state in elements.items():
                ...
                else: raise(AttributeError(f"state -{state} set for -{name} == wrong!"))
            
            for name, state in buttons.items():
                widget = self._buttons[name]
                if state == 'active': widget.active()
                elif state == 'disabled': widget.disable()
                else: raise(AttributeError(f"state -{state} set for button -{name} == wrong!"))
        else:
            #Disable all
